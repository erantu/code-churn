...
curl -s --header "Content-Type: application/json" --header "Accept: application/json" -d "{ \"applicationName\": \"$DIL_REPOSITORY_NAME\", \"applicationVersionBuildNumber\": \"$DIL_REPOSITORY_BRANCH_BUILD_NUMBER\", \"applicationVersionName\": \"$DIL_REPOSITORY_BRANCH\", \"commits\": [ { \"commitId\": \"$DIL_REPOSITORY_BRANCH_GIT_COMMIT_ID\" } ], \"releaseDsl\": { \"dslFilename\": \"$DIL_RELEASE_DSL_FILENAME\", \"dslParameters\": { \"DIL_REPOSITORY_BRANCH\": \"${DIL_REPOSITORY_BRANCH}\" }, \"fileSourceName\": \"$DIL_REPOSITORY_NAME\", \"fileSourceParameters\": {}, \"releaseDslScope\": \"APPLICATION\" } }" "$DEVOPS_SERVER_URL/cdd/design/$DEVOPS_TENANT_ID/v1/applications/application-versions/application-version-builds" -H "Authorization: Bearer $DEVOPS_API_KEY"
